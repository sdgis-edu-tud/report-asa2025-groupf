...
for (col in names(attributes)) {
     plot(grids[col], border = NA, main = attributes[[col]])
}
```
...
cluster_levels <- levels(grids$cluster)
...
plot(grids["cluster"], 
     ...
     border = NA,
     col = cluster_colors[grids$cluster])

legend("topright",
     legend = levels(grids$cluster),
     fill = cluster_colors,
     title = "Cluster",
     cex = 0.9,
     bty = "n")
...
for (i in seq_along(attributes)) {
  colname <- names(attributes)[i]
  display <- attributes[[colname]]
  color <- hist_colors[i]
...
\text{Distance} = \sqrt{
`r paste(
  sapply(names(attributes), function(col) {
    paste0(
...
attr3_names <- names(attributes)[1:3]
attr3_display <- attributes[1:3]
